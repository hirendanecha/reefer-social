...
export function app(): express.Express {
  ...
  const distFolder = join(process.cwd(), 'dist/reefer-social/browser');
  ...
  const template = fs
    .readFileSync(
      path.join(join(process.cwd(), 'dist/reefer-social/browser'), 'index.html')
    )
    .toString();
    ...
    res.render(
      ...
      async (err, html) => {
        ...
        if (
          ...
          const talent = {
            name: `Reefer.social Research ${group?.PageTitle}`,
